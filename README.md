...
**Near-best диапазон k ∈ [35, 50] для Crystal кластеров (R≈160)**: coarse candidate k=35, fine candidate k=50. Подробности — `data/crystal/analysis/k_selection_justification.md`. Согласуется с формулой (5) из [Никифоров и др., 2009]: при R≈160 семейства {111} и выше (g ≤ 8.4 п.р.) полностью умещаются в окно (window_radius=10). Предыдущая рекомендация k=8 отозвана как недооценка.

> **Важно**: R≈160 входит в область применимости модели [Никифоров 2009]
> (R ∈ [100, 1000] п.р.). Универсальность по R проверяется multi-radius ablation
