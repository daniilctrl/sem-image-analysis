"""
Скрипт для извлечения эмбеддингов из обученной SimCLR модели
и сравнения их с Baseline (ImageNet ResNet50) через UMAP и метрики.

Использование:
  python extract_simclr_embeddings.py --checkpoint path/to/simclr_resnet50_epoch_50.pth
"""
import os
import sys
import argparse
...
def main(args):
    ...
    finetuned_emb, names = extract_embeddings(model, loader, device, model_type=args.model_type)

    # 5. Save SimCLR embeddings
    ...
    print(f"\nSaved SimCLR embeddings: {finetuned_emb.shape}")

    # 6. Load baseline embeddings for comparison
    baseline_emb_path = output_path / "resnet50_embeddings.npy"
    baseline_names_path = output_path / "embedding_names.csv"
    ...
    if baseline_emb_path.exists():
        ...
        compute_metrics(simclr_aligned, labels, label_name="SimCLR (Fine-tuned)")

        # 8. UMAP comparison
        build_comparison_umap(
            baseline_aligned, simclr_aligned, labels,
            output_path / "baseline_vs_simclr_umap.png"
        )
    else:
        print(f"\nBaseline embeddings not found at {baseline_emb_path}. Skipping comparison.")
        # Всё равно считаем метрики для SimCLR
        tile_to_group = dict(zip(df['tile_name'], df['group']))
        labels = [tile_to_group.get(n, 'unknown') for n in names]
        compute_metrics(finetuned_emb, labels, label_name="SimCLR (Fine-tuned)")

    print("\nDone!")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract embeddings from trained SimCLR and compare with Baseline")
    parser.add_argument("--checkpoint", type=str, required=True,
                        help="Path to SimCLR .pth checkpoint file")
    ...
    parser.add_argument("--output_dir", type=str, default=r"c:\projects\diploma\data\embeddings")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--workers", type=int, default=0)
    parser.add_argument("--model_type", type=str, default="simclr", choices=["simclr", "byol"],
                        help="Type of model: simclr or byol")
