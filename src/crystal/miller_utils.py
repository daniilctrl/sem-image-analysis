...
# Расширенный набор значимых семейств граней для ОЦК-полусферы.
# Построен по рис. 3 из: Никифоров К.А., Егоров Н.В., Шень Ч.-Ч.,
# «Реконструкция поверхности полевого электронного эмиттера»,
# Поверхность. Рентгеновские, синхротронные и нейтронные исследования,
# 2009, №10, с. 100–106.
#
# В статье (при M=5, R=722 параметра решётки) перечислено ~50+ семейств
# с ненулевой долей поверхностных атомов. Здесь включены все семейства
# с |hkl|² ≤ 35 (28 уникальных), покрывающие доминирующую часть
# поверхности полусферы на всех радиусах интервала 50–1000 п.р.
# Порог |hkl|² ≤ 35 соответствует ожидаемому радиусу грани g ≥ 1 п.р.
# при R=50 по формуле (5) статьи.
#
# Берём канонического представителя каждого семейства (остальные
# эквиваленты — перестановки и знаки — раскрываются get_symmetry_vectors).
_FAMILY_HKL: list[tuple[int, int, int]] = [
    # |hkl|² = 1
    (1, 0, 0),
    # |hkl|² = 2
    (1, 1, 0),
    ...
    # |hkl|² = 35
    (5, 3, 1),
    # часто упоминаемая в статье, |hkl|² = 51
    (7, 1, 1),
]


def _format_family_name(hkl: tuple[int, int, int]) -> str:
    """Каноническое имя семейства: '{hkl}' с сортировкой по убыванию."""
    sorted_hkl = sorted(hkl, reverse=True)
    return "{" + "".join(str(v) for v in sorted_hkl) + "}"


FAMILIES: dict[str, np.ndarray] = {
    _format_family_name(hkl): get_symmetry_vectors(hkl) for hkl in _FAMILY_HKL
}
